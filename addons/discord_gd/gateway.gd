...
func send_payload(data: Dictionary) -> void:
	...
	var err
	if data.op == OpCodes.PRESENCE_UPDATE:
		err = _client.put_packet(payload.substr(1).to_utf8_buffer())
	else:
		err = _client.put_packet(payload.to_utf8_buffer())
...
func _track_latency(latency: int):
